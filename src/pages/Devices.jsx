    ...
    <Box sx={{ width: '100%' }}>
      ...
      {view === 'grid' ? (
        <Grid container spacing={3}>
          {devices.map(de => (
            <Grid item xs={12} sm={6} md={4} key={de.id}>
              <Card sx={{ borderRadius: 3, boxShadow: 2, background: '#fff', transition: '0.2s', '&:hover': { boxShadow: 6, transform: 'translateY(-4px)' } }}>
                <CardContent>
                  <Typography variant="h6" fontWeight={600} gutterBottom>{de.name}</Typography>
                  <Typography variant="body2" color="text.secondary">{de.description}</Typography>
                </CardContent>
                <CardActions sx={{ justifyContent: 'flex-end' }}>
                  <IconButton color="primary" onClick={() => handleOpen(de)}><EditIcon /></IconButton>
                  <IconButton color="error" onClick={() => handleDelete(de.id)}><DeleteIcon /></IconButton>
                </CardActions>
              </Card>
            </Grid>
          ))}
